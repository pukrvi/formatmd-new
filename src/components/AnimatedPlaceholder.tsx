...
const AnimatedPlaceholder = ({ onThemeHint }: AnimatedPlaceholderProps) => {
  ...
  useEffect(() => {
    ...
    if (isTyping) {
      ...
      if (displayText.length > 0) {
        ...
        // Transition to next example synchronously — guard with mounted check
        if (mountedRef.current) {
  ...
  }, [displayText, isTyping, currentIndex, onThemeHint]);

  useEffect(() => {
    onThemeHint(examples[currentIndex].themeIndex);
  }, [currentIndex, onThemeHint]);

  const currentTheme = themes[examples[currentIndex].themeIndex];
